...
class ForceLearningWrapper(gymnasium.Wrapper):
    """
    Adds external forces to the robot base during training to learn robustness.
    Uses the logic from the testing script (set_bullet_action).
    """
    def __init__(self, env, max_force=0.0, push_probability=0.02, push_duration=1.0):
        super().__init__(env)
        self.max_force = max_force  # Curriculum controls this
    ...
    def step(self, action):
        # 1. Management of Push Timer
        if self._push_timer > 0:
            self._push_timer -= self.dt
            if self._push_timer <= 0:
                self._current_force = np.zeros(3) # Stop pushing
                self.unwrapped.set_bullet_action({}) 
        
        # 2. Randomly trigger a new push if none is active
        # We only push if max_force > 0 (Curriculum has started)
        ...
        # 3. Apply the force using Bullet API (Logic from your second script)
        if self._push_timer > 0:
            self.unwrapped.set_bullet_action({
                "external_forces": {
                    "base": {
...
def get_random_word():
    ...
    # Add a unique random suffix (UUID)
